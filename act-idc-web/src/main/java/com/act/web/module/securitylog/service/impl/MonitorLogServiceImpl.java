...
import com.act.idc.model.Ipv4Ipv6Info;
...
/**
 * 
 * <p>Title: MonitorLogServiceImpl.java
 * <p>Description: 监控日志-业务层实现
 * @author zhufj
 * @date 2018年2月2日 下午2:26:41
 */
@Service
@Transactional
public class MonitorLogServiceImpl implements MonitorLogService{
	...
	/**
	 * 
	  * <p>Title: findDetailList
	  * <p>Description:分页查询
	  * @param sessionVo 用户信息session
	  * @param page //分页对象
	  * @param monitorLogVo 监控日志对象vo
	  * @return page //分页对象
	  * @throws Exception 
	 */
	@Override
	public PageResult<MonitorLogVo> findDetailList(SessionVo sessionVo,PageResult<MonitorLogVo> page, MonitorLogVo monitorLogVo)
			throws Exception {
		monitorLogVo.setAccountId(sessionVo.getUserId());
        TabSysConfig tabSysconfig = TabSysConfig.getDao().selectByPrimaryKey("ipv4ToIpv6Set");
		monitorLogVo.setIpv4ToIpv6Set(tabSysconfig.getConfigval());
		//获取选取的时间段并获取到时间段集合
		List<String> dateList = getDateList(monitorLogVo.getStartTimeQuery(), monitorLogVo.getEndTimeQuery());
		//调用查询指令保存日志公共方法
		//获取到数据库表名
		List<String> tableList = hasAlarmData(dateList,"alarm_isms");
		if (tableList.size() > 1) {
			Collections.sort(dateList);
			Collections.reverse(dateList);
		}
		//设置选中日期
		if(!monitorLogVo.getHasSelectTime()){// 选中的日期
			if(tableList.size() > 0){
				monitorLogVo.setSelectTimeQuery(dateList.get(0));
			}else{
				monitorLogVo.setSelectTimeQuery(monitorLogVo.getEndTimeQuery().replace("-", "_"));
			}
		}
		...
		String purService = "";
		//获取查询sql
		String sql_query = getFindSql(page.getPageIndex(),page.getPageSize(),monitorLogVo,monitorLogVo.getSelectTimeQuery(),purService);
		//采用es服务进行搜索并获取esEntity
		EsEntity esEntity =esEntityUtil.getEsEntity(page.getPageIndex(),page.getPageSize(),sql_query,new TypeReference<EsEntity<Hit<List<Hits<AlarmIsmsModel>>>>>(){});
		//分页对象返回
	 ...
	 * @return
	 */
	private String getFindSql(int pageIndex,int pageSize, MonitorLogVo vo,String selectDate,String purService){
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT * from alarm_isms").append("_").append(selectDate).append("where 1=1");
		 //当前时间条件
	 ...
	 */
	@Override
	public Map<String,Object> urlContentViews(MonitorLogVo monitorLogVo) throws Exception {
		Map<String,Object> map = new HashMap<>();
		if(StringUtils.isEmpty(monitorLogVo.getEncodedQuery())){
			monitorLogVo.setEncodedQuery("utf-8");
		}
		//查询快照sql
		StringBuffer urlContent_sql = new StringBuffer("SELECT content FROM  `"+monitorLogVo.getTableNameQuery()+"`");
		urlContent_sql.append("key=").append(monitorLogVo.getHostQuery());
		//es搜索引擎
		EsEntity esEntity = esEntityUtil.getEsEntity(urlContent_sql.toString(),new TypeReference<EsEntity<Hit<List<Hits<AlarmIsmsModel>>>>>(){});
		List<Hits> hits =(List<Hits>)((Hit)esEntity.getHits()).getHits();
		//获取结果
		String content = "";
		...
		if(StringUtils.isEmpty(content)){
			map.put("encoded", monitorLogVo.getEncodedQuery());
			map.put("info", "暂无匹配快照文件");
			return map;
		}
		String info=new String(content.getBytes(),monitorLogVo.getEncodedQuery());
		//获取关键字根据策略
		StringBuffer keyWord_sql = new StringBuffer("SELECT DISTINCT GROUP_CONCAT(valueStart) FROM ismsc_idccommandrule WHERE scheme_id= ").append(monitorLogVo.getRuleIdentifierQuery()).append(" AND subtype =3 GROUP BY scheme_id");
